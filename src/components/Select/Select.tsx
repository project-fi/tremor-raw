...
const selectTriggerStyles = [
  cx(
    // base
    "group/trigger flex w-full select-none items-center justify-between gap-2 truncate rounded-sm border px-3 py-2 outline-none transition sm:text-sm",
    // border color
  ...
  (
    ...
    <SelectPrimitives.Portal>
      <SelectPrimitives.Content
        ...
        className={cx(
          // base
          "relative z-50 overflow-hidden rounded-sm border shadow-xl shadow-black/[2.5%]",
          // widths
      ...
      >
        ...
        <SelectPrimitives.Viewport
          className={cx(
            "p-2 space-y-0.5", // Added space-y-1 for more spacing between options
            position === "popper" &&
...
>(({ className, ...props }, forwardedRef) => (
  <SelectPrimitives.Label
    ...
    className={cx(
      // base
      "px-1 py-2 text-xs font-medium tracking-wide",
      // text color
...
>(({ className, children, ...props }, forwardedRef) => {
  return (
    <SelectPrimitives.Item
      ...
      className={cx(
        // base
        "grid cursor-pointer grid-cols-[1fr_20px] gap-x-2 rounded-sm px-3 py-2 outline-none transition-colors data-[state=checked]:text-blue-700 data-[state=checked]:bg-blue-500/10 sm:text-sm",
        // text color
        "text-gray-900 dark:text-gray-50",
        // disabled
